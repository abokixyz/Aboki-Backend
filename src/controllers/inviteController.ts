...
export const getMyInviteCode = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const myInviteCode = await InviteCode.findOne({ 
      createdBy: user._id 
    });
...
export const createDefaultInviteCode = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const existingCode = await InviteCode.findOne({ code: 'PROJECTINVITE' });
  ...
  try {
    ...
    res.status(200).json({
      ...
      data: {
        ...
        invitedBy: inviteCode.createdBy ? (inviteCode.createdBy as any).username : 'Project'
      }
    });
  } catch (error: any) {
